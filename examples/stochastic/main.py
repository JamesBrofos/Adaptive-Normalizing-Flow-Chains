...
parser = argparse.ArgumentParser(description='Markov chain Monte Carlo with normalizing flows in stochastic differential equations')
parser.add_argument('--target', type=str, default='ou', help='Indicator of which target distribution to use')
...
class ShiftAndLogScale(hk.Module):
    ...
    def __call__(
        ...
        log_scale = jax.nn.tanh(hk.Linear(input_depth)(x))
...
def normalizing_flow_samples(iid: jnp.ndarray, approx: jnp.ndarray):
    ...
    ax.plot(target.t, iid.T, '-', color='tab:blue', alpha=0.03)
    ax.grid(linestyle=':')
    ax.set_xlim((0.0, 20.0))
    ax.set_ylim((-1.0, 11.0))
    ax = fig.add_subplot(122)
    ax.set_title('Normalizing Flow Samples')
    ax.plot(target.t, approx.T, '-', color='tab:blue', alpha=0.03)
    ax.grid(linestyle=':')
    ax.set_xlim((0.0, 20.0))
    ax.set_ylim((-1.0, 11.0))
    fig.tight_layout()
...
def main():
    ...
    curr = jnp.ravel(target.sample(1, next(hkrng)))
    ...
    for i in tqdm.tqdm(range(args.num_steps)):
        ...
        with writer.as_default():
            ...
            if step % (args.num_steps) == 0:
                ...
                fig = kolmogorov_smirnov(iid, A, jax.random.PRNGKey(0))
                im = plot_to_image(fig)
