...
parser = argparse.ArgumentParser(description='Markov chain Monte Carlo with normalizing flows in regression posteriors')
parser.add_argument('--target', type=str, default='hogg', help='Indicator of which target distribution to use')
...
parser.add_argument('--num-steps', type=int, default=50000, help='Number of gradient descent iterations for training')
...
with open('{}-samples.pkl'.format(args.target), 'rb') as f:
...
sample, log_prob = NormalizingFlow(target.num_dims, args.num_hidden)
...
def loss(
    ...
    # Not elegant.
    log_target = jnp.atleast_1d(log_target)
...
def normalizing_flow_density(params: hk.Params):
    xr = jnp.linspace(-1.0, 1.0, 500)
    yr = jnp.linspace(-1.0, 1.0, 500)
    xx, yy = jnp.meshgrid(xr, yr)
    grid = jnp.vstack([jnp.ravel(xx), jnp.ravel(yy)]).T
    tlp = jax.jit(jax.vmap(target.log_prob))(grid)
    prob = jnp.exp(tlp)
...
def adaptive_sample(samples: jnp.ndarray, adapt: jnp.ndarray):
    fig = plt.figure(figsize=(10, 4))
    ax = fig.add_subplot(121)
    ax.set_title('HMC Samples')
    ax.plot(samples[:, 4], samples[:, 6], '.')
    ax.grid(linestyle=':')
    ax.set_xlim((-3.0, 3.0))
    ax.set_ylim((-3.0, 3.0))
    ax = fig.add_subplot(122)
    ax.set_title('Adaptive Samples')
    ax.plot(adapt[:, 4], adapt[:, 6], '.')
    ax.grid(linestyle=':')
    ax.set_xlim((-3.0, 3.0))
    ax.set_ylim((-3.0, 3.0))
    fig.tight_layout()
...
def kolmogorov_smirnov(iid: jnp.ndarray, adapt: jnp.ndarray, rng: jax.random.PRNGKey):
    u = jax.random.normal(rng, [iid.shape[-1]])
    u = u / jnp.linalg.norm(u)
    adaptu = adapt@u
    iidu = iid@u
    fig = plt.figure(figsize=(4, 4))
    ax = fig.add_subplot(111)
    ks = spst.ks_2samp(adaptu, iidu).statistic
    ax.set_title('KS-Test Statistic: {:.4f}'.format(ks))
    ax.hist(adaptu, bins=100, density=True, histtype='step', cumulative=True, label='Adaptive Samples')
    ax.hist(iidu, bins=100, density=True, histtype='step', cumulative=True, label='Independent Samples')
...
def main():
    ...
    params = log_prob.init(next(hkrng), jnp.zeros([target.num_dims]))
        ...
        with writer.as_default():
            ...
            if step % (args.num_steps) == 0:
                ...
                try:
                    ...
                    pass
                try:
                    fig = adaptive_sample(samples, A)
                    im = plot_to_image(fig)
                    tf.summary.image('adaptive samples', im, step=step)
                except:
                    pass
                try:
                    fig = kolmogorov_smirnov(samples, A, jax.random.PRNGKey(0))
                    im = plot_to_image(fig)
