...
f = StringIO("""Survived,Pclass,Name,Sex,Age,Siblings/Spouses Aboard,Parents/Children Aboard,Fare
...
cols = ['Age', 'Siblings/Spouses Aboard', 'Parents/Children Aboard', 'Fare', 'Pclass_1', 'Pclass_2', 'Sex_male']
X = jnp.asarray(data[cols].values)
X = (X - jnp.mean(X, axis=0, keepdims=True)) / jnp.std(X, axis=0, keepdims=True)
X = jnp.hstack([jnp.ones([len(data), 1]), X])
y = jnp.asarray(data['Survived'])
num_dims = X.shape[-1]

dist = tfd.JointDistributionSequential([
    tfd.MultivariateNormalDiag(loc=jnp.zeros(num_dims), scale_identity_multiplier=1.0),
    lambda b: tfd.Independent(
        tfd.Bernoulli(logits=jnp.matmul(X, b)),
...
def main():
    ...
    x, _ = dist.sample(seed=jax.random.PRNGKey(0))
    step_size = 0.1
    samples, sampler_stat = run_chain(x, step_size, tfb.Identity())

    with open('titanic-samples.pkl', 'wb') as f:
